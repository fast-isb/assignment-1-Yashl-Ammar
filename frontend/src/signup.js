...
const SignUp=()=>{
    ...
    function getDetails(){
      if(password!==confirmPassword){
        ...
        axios.post("http://localhost:3001/users/userSignUp",UserSignUp) .then((res)=>{
        console.log(res)
     },(error)=>{
        console.log(error)
    })
    ...
    return(
     ...
     <form className="SignUp Form" onSubmit={getDetails}>
       ...
       <br></br>
       <input type="tel" placeholder="Enter phone num" value={num} onChange={(e)=>setNum(e.target.value)}></input>
       <br></br>
       <input type = "address" placeholder="Address" value={address} onChange={(e)=>setAddress(e.target.value)}></input>
       <br></br>
